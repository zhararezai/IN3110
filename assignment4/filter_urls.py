...
def find_urls(
    ...
    if output:
        ...
        get_html(base_url, output=str(output))

    # first, find all the a tags
    for a_tag in a_pat.findall(html):
        url = href_pat.findall(a_tag)
       
        #print(f"\n\nURL linje 42\n{url}\n")

        if len(url) > 0:
            url = re.split(r"#", url[0])[0] #stripping #-fragments
...
def find_articles(html: str, output=None) -> set:
    ...
    pattern = re.compile("https://en.wikipedia.org\/wiki\/\w+") #regex pattern: \/wiki\/\w+
    articles = set()

    for url in urls:
        #print(url)
        if re.search(pattern, url): #hvis pattern finnes i url
            #print(f"\nregex found pattern in url {url}\n")
            articles.add(url)
